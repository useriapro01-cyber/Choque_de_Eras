...
- 🔨 TDMV-4: PWA + deploy automático. **Feito (jul/2026):** `npm run build` emite PWA em `dist/` — `index.html` self-contained (+ cópia `choque-de-eras.html`), `manifest.webmanifest`, `sw.js` (cache-first offline, versão carimbada pelo hash do conteúdo), ícones 192/512/maskable/apple-touch (fonte em `src/pwa/`, gerados via `scripts/gerar-icones.mjs` — precisa de `@resvg/resvg-js` só para regenerar). `vercel.json` fixa build/saída/headers; `tests/pwa.test.mjs` cobre o wiring. **Pendente do founder:** import único do repo no painel Vercel (Root Directory = `repo-starter`, branch de produção `main`) para ligar o auto-deploy por push.
