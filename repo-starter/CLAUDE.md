...
3. **Determinismo por seed.** Mesma seed + mesmas decisões = mesmo resultado, sempre (base do Desafio do Dia, das salas e da validação anti-fraude server-side por replay). Nunca usar Math.random direto no fluxo de campanha; sempre o RNG semeado. **Cumprido de fato desde o TDMV-5 Fase A** (jul/2026): toda a partida/eventos passou a ser semeada por `seed+rodada` no motor puro `src/engine.js`; garantido pelo teste de paridade (`tests/paridade.test.mjs`). O `Math.random` só sobrevive na UI para gerar seeds novas e código de sala.
