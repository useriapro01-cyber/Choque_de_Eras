...
- 🔨 TDMV-4: PWA + deploy automático. **Feito (jul/2026):** `npm run build` emite PWA em `dist/` — `index.html` self-contained (+ cópia `choque-de-eras.html`), `manifest.webmanifest`, `sw.js` (cache-first offline, versão carimbada pelo hash do conteúdo), ícones 192/512/maskable/apple-touch (fonte em `src/pwa/`, gerados via `scripts/gerar-icones.mjs` — precisa de `@resvg/resvg-js` só para regenerar). `vercel.json` na **raiz do repo** é a fonte de verdade do deploy (`cd repo-starter && npm run build`, output `repo-starter/dist`, `framework:null`) — funciona no setup padrão sem tocar no painel; há um segundo `vercel.json` em `repo-starter/` para o caso de Root Directory = `repo-starter`. `tests/pwa.test.mjs` cobre o wiring. **Pendente do founder:** import único do repo na Vercel (só Import → Deploy, branch `main`) para ligar o auto-deploy por push.
